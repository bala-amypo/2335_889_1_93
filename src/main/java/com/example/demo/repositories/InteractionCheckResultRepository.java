...
public interface InteractionCheckResultRepository extends JpaRepository<InteractionCheckResult, Long> {
    ...
    // Find results with highest interaction counts
    ...
    // Find latest result for a specific user
    @Query("SELECT icr FROM InteractionCheckResult icr WHERE icr.userId = :userId ORDER BY icr.checkedAt DESC LIMIT 1")
    InteractionCheckResult findLatestByUserId(@Param("userId") Long userId);
}